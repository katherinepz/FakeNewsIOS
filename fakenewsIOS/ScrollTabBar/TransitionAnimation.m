...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    [UIView animateWithDuration:transitionDuration animations:^{
        ...
        if ([transitionContext transitionWasCancelled]) {
            if ([_delegate respondsToSelector:@selector(TransitionDidEndToViewController:)])
            {
                [_delegate TransitionDidEndToViewController:fromViewController];
            }

        } else {
            if ([_delegate respondsToSelector:@selector(TransitionDidEndToViewController:)])
            {
                [_delegate TransitionDidEndToViewController:toViewController];
            }
        }
    }];
}
