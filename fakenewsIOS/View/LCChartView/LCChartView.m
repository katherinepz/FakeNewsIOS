...
- (void)drawYAxis {
    ...
    for (int i = 0; i < _yAxisCount + 1; i++) {
        ...
        NSString *title = [NSString stringWithFormat:@"%.1f%%", avgValue * i];
        ...
        label.LC_width = _yAxisToLeft - _yTextToAxis + 15;
...
- (void)drawBarChartViewBars {
    ...
    for (int i = 0; i < self.dataSource.count; i++) {
        ...
        for (int j = 0; j < model.plots.count; j++) {
            ...
            CAShapeLayer *barShapeLayer = nil;
            if (j != model.hightlightIndex) {
                barShapeLayer = [self shapeLayerWithPath:barPath lineWidth:_barWidth fillColor:model.color strokeColor:model.color];
            }
            else
            {
                barShapeLayer = [self shapeLayerWithPath:barPath lineWidth:_barWidth fillColor:model.hightlightColor strokeColor:model.hightlightColor];
            }
            [self.scondLayers addObject:barShapeLayer];
            UILabel *lblTitle = [[UILabel alloc] initWithFrame:CGRectMake(startPointX - _barWidth / 2,  [self getValueHeightWith:model.plots[j]] - 14, _barWidth, 14)];
            lblTitle.text = model.plots[j];
